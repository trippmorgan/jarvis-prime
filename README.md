...
| **W21** | Two recurring runs Tripp validated — the X post and the 2026-05-18 morning show — codified as first-class orchestrated **systems processes** (see [`SYSTEMS-PROCESSES.md`](SYSTEMS-PROCESSES.md)). Process A (X post): general classifier/plan, 2-step `social-draft`(T1, auto) → `social-post`(T3 gate). Process B (morning show): new `morning-show-build`(T1) → `morning-show-publish`(T3) plan + prime-only `cmd_morning_show` substrate (plan-mode default — inert until deliberately triggered). The T3 gate now **renders the draft / preview before the confirm prompt** (fixes the "generic/unusable" gate the SuperServer diagnosis flagged). | "draft a tweet about the morning show" / "build the morning show" now orchestrate with a reviewable confirm gate instead of falling to chat. +13 tests (650 total). |
