...
import { parseDispatch, parseLeftToolsEvidence } from "./dispatch-parser.js";
...
//                    Buckets intentionally remain stable across timeout-budget
//                    tuning. The current fast cap is commonly 90s, so active
//                    timeouts usually land in "60-120s"; older 180s-era traces
//                    land in "180s-timeout". The terminal label is historical
//                    and means elapsed >=180s, not necessarily the live budget.
//   - timeout_ms     configured per-left-call timeout budget for this dispatch
//   - timeout_phase  omitted unless a left timeout occurred; then one of
//                    "pass1" | "pass2" | "integration"
...
export interface DualBrainDispatchTelemetry {
  ...
  timeout_ms: number;
  timeout_phase?: "pass1" | "pass2" | "integration";
}
...
export async function corpusCallosum(
  ...
  let telemetryTimeoutPhase: "pass1" | "pass2" | "integration" | undefined;
  ...
  const noteLeftTimeout = (
    ...
    if (
      ...
      telemetryTimeoutPhase = phase;
    }
  };

  const callLeft = async (
    phase: "pass1" | "pass2" | "integration",
    input: Parameters<HemisphereClient["call"]>[0],
  ): ReturnType<HemisphereClient["call"]> => {
    try {
      return await left.call(input);
    } catch (err) {
      noteLeftTimeout(phase, err);
      throw err;
    }
  };
  ...
  try {
    ...
    if (routerEnabled) {
      ...
      p1LeftResult = await callLeft("pass1", {
    ...
    const [p2LeftResult, p2RightResult] = await Promise.all([
      ...
      right.call({
        system: p2RightPrompt.system,
        user: p2RightPrompt.user,
        timeoutMs,
      }),
    ]);
    ...
    try {
      const first = await callLeft("integration", {
    ...
    if (routerEnabled) {
      ...
      if (firstCheck && !firstCheck.adequate && firstCheck.gaps.length > 0) {
        ...
        try {
          const retry = await callLeft("integration", {
